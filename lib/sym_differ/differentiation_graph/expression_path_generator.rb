...
module SymDiffer
  module DifferentiationGraph
    ...
    class ExpressionPathGenerator
      ...
      def build_expression_points_for_expression(step_range, current_path = [])
        return current_path unless first_element_of_range(step_range) <= second_element_of_range(step_range)
        ...
        build_expression_points_for_expression(new_step_range, current_path + [evaluation_point_for_current_step])
      end
      ...
      def build_evaluation_point(ordinate, abscissa)
        [ordinate, abscissa]
      end
